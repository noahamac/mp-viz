...
var utils_formatType = function (valueFormat) {
    if (!valueFormat)
        return undefined;
    var format = "";
...
// EXTERNAL MODULE: ./node_modules/ssf/ssf.js
var ssf = __webpack_require__(38);
...
function linkTooltip(d, event, dragging, measure, valFormat) {
    if (!dragging) {
        ...
        if (measure) {
            html += "<br>" + (measure.label_short || measure.label) + ": <b>" + Object(ssf["format"])(valFormat, d.value) + "</b>";
        }
        src_select("#tooltip")
...
function _clear(onHoverLabels, focus) {
    if (focus) {
        src_selectAll("line").attr("opacity", 0.1);
        src_selectAll("circle").attr("opacity", 0.2);
        src_selectAll("text").attr("opacity", !onHoverLabels ? 0.2 : 0);
    }
}
function highlightNeighbors(d, event, dragging, onHoverLabels, focus) {
    if (!dragging) {
        _clear(onHoverLabels, focus);
        var link_target = src_selectAll("line[target=\"" + d.id + "\"]");
        var link_source = src_selectAll("line[source=\"" + d.id + "\"]");
        link_target.attr("opacity", 1);
        link_source.attr("opacity", 1);
        src_select("circle[id=\"" + d.id + "\"]").attr("opacity", 1);
        src_select("text[id=\"" + d.id + "\"]").attr("opacity", 1);
        link_target.each(function (e) {
            src_select("circle[id=\"" + e.source.id + "\"]").attr("opacity", 1);
            src_select("text[id=\"" + e.source.id + "\"]").attr("opacity", 1);
        });
        link_source.each(function (e) {
            src_select("circle[id=\"" + e.target.id + "\"]").attr("opacity", 1);
            src_select("text[id=\"" + e.target.id + "\"]").attr("opacity", 1);
        });
    }
}
function hideTooltip() {
    src_select("#tooltip").style("opacity", 0).style("left", 0);
}
function clearHighlight(onHoverLabels) {
    src_selectAll("line").attr("opacity", 1);
    src_selectAll("circle").attr("opacity", 1);
    src_selectAll("text").attr("opacity", onHoverLabels ? 0 : 1);
}
function yOffset() {
    return parseInt(src_select("#tooltip").style("height"), 10) + yPadding;
}

// CONCATENATED MODULE: ./src/force-directed.ts
...
var vis = {
    ...
    create: function (element, config) {
        ...
        this.svg = src_select(element).append("svg");
    },
    ...
    updateAsync: function (data, element, config, queryResponse, details, done) {
        ...
        var applyDefualtConfig = function () {
            for (var option in _this.options) {
                if (config[option] === undefined) {
                    config[option] = _this.options[option].default;
                }
            }
        };
        ...
        var SOURCE = "SOURCE";
        var TARGET = "TARGET";
        var graphOptions = [NODE_1, GROUP_1, NODE_2, GROUP_2];
        var labels = [
            "Source nodes",
            "Source groups",
            "Target nodes",
        ...
        var generateOptions = function (dimensions) {
            var newOptions = Object.assign({}, _this.options);
            var selections = dimensions.map(function (d) {
                var _a;
                return (_a = {},
                    _a[d.label_short || d.label] = d.name,
                    _a);
            });
            graphOptions.forEach(function (g, i) {
                newOptions[g] = {
                    section: "Graph",
                    label: labels[i],
                    type: "string",
            ...
            _this.trigger("registerOptions", newOptions);
        };
        var height = element.clientHeight + 20;
        var width = element.clientWidth;
        var radius = config.circle_radius;
        var linkDistance = config.linkDistance;
        var dimensions = queryResponse.fields.dimension_like;
        var measure = queryResponse.fields.measure_like[0] || null;
        var valFormat = (measure === null || measure === void 0 ? void 0 : measure.value_format) || (config === null || config === void 0 ? void 0 : config.tooltipValFormat) || "#,###";
        ...
        var colorScale = ordinal();
        var color = colorScale.range(category10);
        ...
        data.forEach(function (row) {
            var _a, _b;
            if (row[config[NODE_1]] === undefined ||
                row[config[NODE_2]] === undefined ||
                ((_a = row[config[NODE_1]]) === null || _a === void 0 ? void 0 : _a.value) === null ||
                ((_b = row[config[NODE_2]]) === null || _b === void 0 ? void 0 : _b.value) === null) {
                return;
            }
            if (nodes_unique.indexOf(row[config[NODE_1]].value) == -1) {
                ...
                var nodeDim = dimensions.find(function (e) { return e.name === config[NODE_1]; });
                var groupDim = dimensions.find(function (e) { return e.name === config[GROUP_1]; });
                var newnode = {
            ...
            if (nodes_unique.indexOf(row[config[NODE_2]].value) == -1) {
                ...
                var nodeDim = dimensions.find(function (e) { return e.name === config[NODE_2]; });
                var groupDim = dimensions.find(function (e) { return e.name === config[GROUP_2]; });
                var newnode = {
        ...
        //manybody.strength(-7)
        var simulation = src_simulation(nodes)
            .force("link", src_link(links)
            .distance(linkDistance)
            .id(function (d) { return d.id; }))
            .force("charge", manybody)
            .force("center", src_center(width / 2, height / 2));
        var svg = this.svg.attr("width", "100%").attr("height", height);
        var link = svg
            .append("g")
            ...
            var func = {
                sqrt: function (d) { return Math.sqrt(d); },
                cbrt: function (d) { return Math.cbrt(d); },
                log2: function (d) { return Math.log2(d); },
                log10: function (d) { return Math.log10(d); },
            };
            if (measure) {
            ...
            src_select(this).attr("stroke", "#FFA500");
            if (config.tooltip) {
                linkTooltip(d, on_event, isDragging, measure, valFormat);
            }
        })
            .on("mousemove", function (d) {
            if (config.tooltip) {
                updatePosition(d, on_event, isDragging, "link");
            }
        })
            .on("mouseleave", function () {
            src_select(this).attr("stroke", config.link_color);
            hideTooltip();
        });
        var node = svg
            .append("g")
        ...
        var circle = node
            ...
            .on("mouseover", function (d) {
            src_select(this).attr("stroke-width", 1.5).attr("stroke", "#FFA500");
            if (config.tooltip) {
                nodeTooltip(d, on_event, isDragging);
            }
            if (config.highlight_selection || config.labels === "on_hover") {
                highlightNeighbors(d, on_event, isDragging, config.labels === "on_hover", config.highlight_selection);
            }
        })
            .on("mousemove", function (d) {
            if (config.tooltip) {
                updatePosition(d, on_event, isDragging, "node");
            }
        })
            .on("mouseleave mousedown", function () {
            src_select(this).attr("stroke-width", 1).attr("stroke", "#fff");
            hideTooltip();
            if (config.highlight_selection || config.labels === "on_hover") {
        ...
        if (config.labelTypes && config.labelTypes.length) {
            labelTypes = config.labelTypes.split(",").map(function (e) { return e.trim(); });
        }
        ...
        if (config.labels === "on_hover") {
            src_selectAll("text").attr("opacity", 0);
        }
